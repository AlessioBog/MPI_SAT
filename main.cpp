...
const int MAX_CLAUSES = 10;
const int MAX_LITS = 5;
const int MAX_VARS = 10;

int formula1[MAX_CLAUSES][MAX_LITS] = {
    {1, 2, 0},
    {-1, 0},
    {-2, 0}
};
...
bool containsVar1(int clause[], int v) {
    for (int i = 0; clause[i] != 0; ++i)
        if (clause[i] == v)
            return true;
    return false;
...
void resolveClauses1(int a[], int b[], int v, int out[]) {
    int k = 0;
    for (int i = 0; a[i] != 0; ++i)
        if (a[i] != v)
            out[k++] = a[i];
...
bool dp1(int vars[], int varCount) {
    for (int vi = 0; vi < varCount; ++vi) {
        int v = vars[vi];
        int newClauses[MAX_CLAUSES][MAX_LITS];
        int newCount = 0;

        // Separate clauses
        for (int i = 0; i < numClauses1; ++i) {
            if (containsVar1(formula1[i], v) || containsVar1(formula1[i], -v))
                continue;
            for (int j = 0; formula1[i][j] != 0; ++j)
                newClauses[newCount][j] = formula1[i][j];
            newCount++;
        }

        // Resolve v
        for (int i = 0; i < numClauses1; ++i) {
            if (!containsVar1(formula1[i], v)) continue;
            for (int j = 0; j < numClauses1; ++j) {
                if (!containsVar1(formula1[j], -v)) continue;
                int resolvent[MAX_LITS];
                resolveClauses1(formula1[i], formula1[j], v, resolvent);
                if (!isTautology1(resolvent)) {
                    for (int k = 0; resolvent[k] != 0; ++k)
                        newClauses[newCount][k] = resolvent[k];
                    newCount++;
                }
            }
        }
        ...
        // Copy back
        numClauses1 = newCount;
        for (int i = 0; i < numClauses1; ++i)
            for (int j = 0; j < MAX_LITS; ++j)
                formula1[i][j] = newClauses[i][j];
...
int formula[MAX_CLAUSES][MAX_LITS] = {
    {1, 2, 0},
    {-1, 0},
    {-2, 0}
};
...
bool containsVar(int clause[], int v) {
    for (int i = 0; clause[i] != 0; ++i)
        if (clause[i] == v)
            return true;
    return false;
...
void resolveClauses(int a[], int b[], int v, int out[]) {
    int k = 0;
    for (int i = 0; a[i] != 0; ++i)
        if (a[i] != v)
            out[k++] = a[i];
...
bool dp(int vars[], int varCount) {
    for (int vi = 0; vi < varCount; ++vi) {
        ...
        for (int i = 0; i < numClauses; ++i) {
            if (containsVar(formula[i], v) || containsVar(formula[i], -v))
                continue;
            for (int j = 0; formula[i][j] != 0; ++j)
                newClauses[newCount][j] = formula[i][j];
            newCount++;
        }

        // Resolve v
        for (int i = 0; i < numClauses; ++i) {
            if (!containsVar(formula[i], v)) continue;
            for (int j = 0; j < numClauses; ++j) {
                if (!containsVar(formula[j], -v)) continue;
                int resolvent[MAX_LITS];
                resolveClauses(formula[i], formula[j], v, resolvent);
...
int formula3[MAX_CLAUSES][MAX_LITS] = {
    {1, 0},
    {-1, 0}
};
int numClauses3 = 2;

bool isEmptyClause(int clause[]) {
    return clause[0] == 0;
}

bool resolve(int a[], int b[], int out[]) {
    int pivot = 0;
    for (int i = 0; a[i] != 0; ++i)
        for (int j = 0; b[j] != 0; ++j)
            if (a[i] == -b[j])
...
int main() {
    int vars1[MAX_VARS] = {1, 2};
    int varCount1 = 2;

    if (dp1(vars1, varCount1))
        cout << "SAT (DP)\n";
    else
        cout << "UNSAT (DP)\n";
        ...
        int newClause[MAX_LITS];
    bool derivedEmpty = false;

    for (int i = 0; i < numClauses3; ++i) {
        for (int j = i + 1; j < numClauses3; ++j) {
            if (resolve(formula3[i], formula3[j], newClause)) {
                if (isEmptyClause(newClause)) {
                    derivedEmpty = true;
                    break;
                }
            }
        }
        if (derivedEmpty) break;
    }

    if (derivedEmpty)
        cout << "UNSAT (Resolution)\n";
    else
        cout << "SAT or Unknown (Resolution)\n";




        int vars[MAX_VARS] = {1, 2};
    int varCount = 2;
